...
class abs_spectra(): 
  ...
  def baseline(self,baseline_path = None, nm = None):
    ...
    if baseline_path:
      if not (self.corrected_blank):
        ...
        self.corrected_blank = True
      else:
        print('You have already substracted the buffer!')
    

    if nm:
      if not self.corrected_wv:
        idx = self.wavelength[self.wavelength == nm].index
        self.abs = self.abs - float(self.abs.iloc[idx])
        self.corrected_wv = True
      else:
  ...
  def concentration_calc (self, wv, molar_extinction_coeff, pathlength):
    '''Calculates the concentration of the molecule using its molar extinction coefficient at a fixed wavelength and the optical pathlenght.
      ...
      pathlenght: cm
      '''
    idx = self.wavelength[self.wavelength == wv].index
    abs = float(self.abs.iloc[idx])
